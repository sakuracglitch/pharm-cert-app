...
function renderHistory(){
  ...
  return `<section class="history-tabs"><button class="${historyMode==='timeline'?'on':''}" onclick="setHistoryMode('timeline')">研修履歴</button><button class="${historyMode==='matrix'?'on':''}" onclick="setHistoryMode('matrix')">項目別一覧</button></section>${count}${historyMode==='timeline'?renderHistoryTimeline():renderItemMatrix()}`
}
...
function saveConferenceTraining(){
  ...
  const trainingId=uid(),conferenceId=conferenceDraft.id;
  state.trainings.push({id:trainingId,date:conferenceDraft.date,name:conferenceDraft.name,conferenceId,source:'jshp',cpc:false,hopessId:'',memo:'学会プログラムから一括登録',files:conferenceDraft.fileName?[{name:conferenceDraft.fileName}]:[],creditEntries:entries});
